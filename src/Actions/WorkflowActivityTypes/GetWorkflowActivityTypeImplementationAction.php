...
class GetWorkflowActivityTypeImplementationAction
{
    /**
    ...
    public function handle(WorkflowActivityType|int|string $workflowActivityType, array $parameters = []): WorkflowActivityTypeContract
    {
        ...
        if (! cache()->has($cacheKey)) {
            /** @var CacheWorkflowActivityTypeImplementationsAction $cacheAction */
            $cacheAction = app(CacheWorkflowActivityTypeImplementationsAction::class);
            $cacheAction->handle();
        }
        ...
        if (! isset($workflowActivityTypeContracts[$workflowActivityTypeId])) {
            /** @var CacheWorkflowActivityTypeImplementationsAction $cacheAction */
            $cacheAction = app(CacheWorkflowActivityTypeImplementationsAction::class);
            $workflowActivityTypeContracts = $cacheAction->handle();
        }
